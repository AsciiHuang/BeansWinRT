...
namespace Beans
{
    ...
    public sealed partial class MainPage : Page
    {
        ...
        private Boolean mStupidMode = false;
        ...
        public MainPage()
        {
            ...
            timerMessageHide.Tick += OnShowTimerTick;
        }

        private void InitBensCount()
        {
            mStupidMode = Utility.IsStupidMode();
        ...
        private void OnShowTimerTick(object sender, object e)
        {
        ...
        private void ShowMessage(String msg, Boolean autoHide)
        {
            ...
            Storyboard.SetTargetProperty(animX, "panel.ScaleX");
            Storyboard.SetTargetProperty(animY, "panel.ScaleY");
        ...
        {
            SettingsPane.GetForCurrentView().CommandsRequested += OnSettingCommandsRequested;
            SwitchBGMusic();
        }
        ...
        Popup mOptionsPopup = null;
        private void OnSettingCommandsRequested(SettingsPane sender, SettingsPaneCommandsRequestedEventArgs args)
        {
            SettingsCommand cmdMember = new SettingsCommand("options", "Game Options", (x) =>
            {
                Rect bounds = Window.Current.Bounds;
                mOptionsPopup = new Popup();
                mOptionsPopup.IsLightDismissEnabled = true;
                mOptionsPopup.Width = 346;
                mOptionsPopup.Height = bounds.Height;
                mOptionsPopup.Closed += OnPopupClosed;
                Window.Current.Activated += OnWindowActivated;

                OptionsPage mypane = new OptionsPage();
                mypane.OnOptionPageMusicEvent += SwitchBGMusic;
                mypane.OnOptionPageModeEvent += SwitchGameMode;
                mypane.OnOptionPagePathModeEvent += SwitchPathMode;
                mypane.Width = 346;
                mypane.Height = bounds.Height;

                mOptionsPopup.Child = mypane;
                mOptionsPopup.SetValue(Canvas.LeftProperty, bounds.Width - 346);
                mOptionsPopup.SetValue(Canvas.TopProperty, 0);
                mOptionsPopup.IsOpen = true;
            });
            args.Request.ApplicationCommands.Add(cmdMember);
        }

        private void SwitchPathMode()
        {
            mStupidMode = Utility.IsStupidMode();
        }
        ...
        private void SwitchBGMusic()
        {
            ...
            if (bBGMusicValid)
            {
                BeansMediaElement.Source = new Uri("ms-appx:///Assets/bgm.mp3");
            }
            else
            {
                BeansMediaElement.Stop();
            }
        }
        ...
        private void OnGameSpacePressed(object sender, PointerRoutedEventArgs e)
        {
            // 先依選中的格子來判斷走什麼流程
            ...
            PointerPoint ppt = e.GetCurrentPoint(GameSpace);
            Position ptVisual = GetCoordinate(ppt.Position); // 回傳為視覺 left、top 座標
            if (ptVisual.X >= 0)
            {
        ...
        private void OnHowToPlayButtonClick(object sender, RoutedEventArgs e)
        {
            BottomAppBar.IsOpen = false;
            ScaleTransform panel = TutorialPanel.RenderTransform as ScaleTransform;
            DoubleAnimation animX = new DoubleAnimation();
            animX.From = 0;
            animX.To = 1;
            animX.Duration = new Duration(TimeSpan.FromSeconds(0.2));
        ...
        private void OnTutorialPanelTapped(object sender, TappedRoutedEventArgs e)
        {
            ScaleTransform panel = TutorialPanel.RenderTransform as ScaleTransform;
            DoubleAnimation animX = new DoubleAnimation();
            animX.From = 1;
        ...
        private BitmapImage GetBeanImagePath(int type)
        {
            ...
            return new BitmapImage(new Uri(strPath));
        }
        ...
        private void ShowVisitAnimation(int beanType)
        {
            ...
            Storyboard.SetTargetProperty(frames, "RoleBean.Center");
            frames.EnableDependentAnimation = true;
        ...
        void OnRemoveBeanStoryboardCompleted(object sender, object e)
        {
